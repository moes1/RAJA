...
class ReduceSumROCM : public ::testing::Test
{
public:
  static void SetUpTestCase()
  {

    rocmMalloc((void **)&dvalue,
...
double* ReduceSumROCM::dvalue = nullptr;
double* ReduceSumROCM::rand_dvalue = nullptr;
int* ReduceSumROCM::ivalue = nullptr;

const size_t block_size = 256;

ROCM_TEST_F(ReduceSumROCM, staggered_sum)
{
  double* dvalue = ReduceSumROCM::dvalue;

  double dtinit = 5.0;

  ReduceSum<rocm_reduce<block_size>, double> dsum0(0.0);
  ReduceSum<rocm_reduce<block_size>, double> dsum1(dtinit * 1.0);
  ReduceSum<rocm_reduce<block_size>, double> dsum2(0.0);
  ReduceSum<rocm_reduce<block_size>, double> dsum3(dtinit * 3.0);
  ReduceSum<rocm_reduce<block_size>, double> dsum4(0.0);
  ReduceSum<rocm_reduce<block_size>, double> dsum5(dtinit * 5.0);
  ...
  for (int k = 0; k < loops; k++) {
    ...
    forall<rocm_exec<block_size> >(0, TEST_VEC_LEN, [=] __device__(int i) {
      dsum0 += dvalue[i];
      dsum1 += dvalue[i] * 2.0;
      dsum2 += dvalue[i] * 3.0;
      dsum3 += dvalue[i] * 4.0;
...
ROCM_TEST_F(ReduceSumROCM, indexset_aligned)
{
  double* dvalue = ReduceSumROCM::dvalue;
  int* ivalue = ReduceSumROCM::ivalue;
  ...
  ReduceSum<rocm_reduce<block_size>, double> dsum0(dtinit * 1.0);
  ReduceSum<rocm_reduce<block_size>, int> isum1(itinit * 2);
  ReduceSum<rocm_reduce<block_size>, double> dsum2(dtinit * 3.0);
  ReduceSum<rocm_reduce<block_size>, int> isum3(itinit * 4);

  forallN<NestedPolicy<ExecList<ExecPolicy<seq_segit,
                                           rocm_exec<block_size> > > > >(
...
//
ROCM_TEST_F(ReduceSumROCM, indexset_noalign)
{
  double* dvalue = ReduceSumROCM::dvalue;
  int* ivalue = ReduceSumROCM::ivalue;
  ...
  forall<ExecPolicy<seq_segit, rocm_exec<block_size> > >(
      iset, [=] __device__(int i) {
        dsum0 += dvalue[i];
        isum1 += 2 * ivalue[i];
        dsum2 += 3 * dvalue[i];
...
ROCM_TEST_F(ReduceSumROCM, atomic_reduce)
{
  double* rand_dvalue = ReduceSumROCM::rand_dvalue;

  ReduceSum<rocm_reduce_atomic<block_size>, double> dsumN(0.0);
  ReduceSum<rocm_reduce_atomic<block_size>, double> dsumP(0.0);
...
ROCM_TEST_F(ReduceSumROCM, increasing_size)
{
  double* dvalue = ReduceSumROCM::dvalue;
  ...
  for (int size = block_size; size <= TEST_VEC_LEN; size+=block_size ) {
    ...
    ReduceSum<rocm_reduce<block_size, true>, double> dsum0(dtinit);

    forall<rocm_exec<block_size, true> >(0, size, [=] __device__(int i) {
      dsum0 += dvalue[i];
    });
