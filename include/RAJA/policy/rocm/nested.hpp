...
#ifndef RAJA_policy_rocm_nested_HPP
#define RAJA_policy_rocm_nested_HPP
...
template <template <camp::idx_t, typename...> class ForTypeIn,
          ...
          camp::idx_t Index,
          typename... Rest>
struct Executor<ForTypeIn<Index, rocm_exec<block_size>, Rest...>> {
  using ForType = ForTypeIn<Index, rocm_exec<block_size>, Rest...>;
...
template <template <camp::idx_t, typename...> class ForTypeIn,
          camp::idx_t Index,
          typename... Rest>
struct Executor<ForTypeIn<Index, rocm_loop_exec, Rest...>> {
  using ForType = ForTypeIn<Index, rocm_loop_exec, Rest...>;
  static_assert(std::is_base_of<internal::ForBase, ForType>::value,
                "Only For-based policies should get here");
  ...
  void RAJA_DEVICE operator()(ForType const &fp, WrappedBody const &wrap)
  {
    ...
    using ::RAJA::policy::rocm::forall_impl;
    forall_impl(fp.exec_policy,
                camp::get<ForType::index_val>(wrap.data.segment_tuple),
                ForWrapper<WrappedBody>{wrap});
  }
...
[[hc]] void rocmLauncher(BODY loop_body)
{
...
struct ROCmWrapper {
  ...
  using Next = ROCmWrapper<idx + 1, n_policies, Data>;
  using data_type = typename std::remove_reference<Data>::type;
  Data &data;

  explicit RAJA_DEVICE ROCmWrapper(Data &d) : data{d} {}
...
struct ROCmWrapper<n_policies, n_policies, Data> {
  constexpr static int cur_policy = n_policies;
  constexpr static int num_policies = n_policies;
  using Next = ROCmWrapper<n_policies, n_policies, Data>;
  using data_type = typename std::remove_reference<Data>::type;
  Data &data;
...
struct rocm_collapse_exec : public rocm_exec<0, Async> {
};


template <typename... FOR>
using ROCmCollapse = Collapse<rocm_collapse_exec<false>, FOR...>;

template <typename... FOR>
using ROCmCollapseAsync = Collapse<rocm_collapse_exec<true>, FOR...>;


// TODO, check that FT... are rocm policies
template <bool Async, typename... FOR_TYPES>
struct Executor<Collapse<rocm_collapse_exec<Async>, FOR_TYPES...>> {

  using collapse_policy = Collapse<rocm_collapse_exec<Async>, FOR_TYPES...>;
  ...
  struct ForWrapper {
    ...
    using RoWrap = internal::ROCmWrapper<BaseWrapper::cur_policy,
  ...
  void operator()(collapse_policy const &, WrappedBody const &wrap)
  {
    ...
    /* As we create a rocm wrapper, we construct all of the rocm loop policies,
     * like rocm_thread_x_exec, their associated segment from wrap.data.segment_tuple
     *
     * This construction modifies the ROCmDim to specify the correct number
    ...
    */
    ...
    auto rocm_wrap =
        ForWrapper<WrappedBody,
                   ...
                   typename FOR_TYPES::policy_type::rocm_exec_policy...>(
            wrap,
    ...
    if (numBlocks(dims) > 0 && numThreads(dims) > 0) {
      ...
      internal::rocmLauncher(
          RAJA::rocm::make_launch_body(
              dims.num_blocks, dims.num_threads, 0, stream, rocm_wrap));
      }).wait();
